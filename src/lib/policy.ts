...
export function classifyGatehouseTarget(
  ...
  if (!secure && !options.allowHttp) {
    return {
      ...
      origin: url.origin,
      secure: false,
      reason: 'Insecure HTTP pages are not embedded inside Gatehouse.',
      allowSameOrigin: false
    }
  }
  ...
  if (options.embedOrigins.has(url.origin)) {
    return {
      ...
      origin: url.origin,
      secure,
      reason: 'You have explicitly trusted this origin for embedded use.',
  ...
  return {
    ...
    origin: url.origin,
    secure,
    reason: 'This site is not on your trusted-origins list.',
    allowSameOrigin: false
  }
}
...
/**
 * Builds a user's embed-origin set from their saved trusted-origin rows plus
 * the app's own current origin (so Gatehouse can always embed itself).
 * Deliberately takes a plain string array (DB rows), not an env var - unlike
 * the original single-tenant RideArrivo build, trust here is per-user data,
 * not a build-time global, because one user's decision to trust an origin
 * must never make that origin embeddable for every other Gatehouse user.
 */
...
/**
 * Subset of a user's trusted origins that they've also opted into keeping
